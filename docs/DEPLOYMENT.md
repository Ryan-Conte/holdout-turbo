...
- Add capacity by creating another named Render service with a distinsAWwct `PUBLIC_SERVER_URL`. It automatically appears in the landing-page server browser.
- Published maps and engine content are shared through Postgres. Each API polls for revisions; a published map is adopted when that server's world is empty.
