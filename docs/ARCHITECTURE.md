...
- Mobile play requests fullscreen/landscape during deployment and enforces a portrait gate where orientation lock is unavailable. `MobileControls.tsx` supplies independently pointer-captured movement and aim/fire sticks plus touch actions/navigation; `GameClient.tsx` merges those directions with keyboard state into the existing `InputPayload`, so mobile never gains position, combat or inventory authority. Safe-area layout and the ongoing feature checklist are documented in `docs/MOBILE.md`.
