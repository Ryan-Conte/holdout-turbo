...
const OUT = path.join(path.dirname(fileURLToPath(import.meta.url)), '..', 'apps', 'web', 'public', 'sprites');
...
function rect(png, x, y, w, h, color) {
  for (let j = y; j < y + h; j++) for (let i = x; i < x + w; i++) px(png, i, j, color);
}
...
function speckle(png, cx, cy, colors, count, seed) {
  ...
  for (let i = 0; i < count; i++) {
    px(png, cx + Math.floor(r() * C), cy + Math.floor(r() * C), colors[i % colors.length]);
  }
}

function save(png, name) {
  png.pack().pipe(fs.createWriteStream(path.join(OUT, name))).on('finish', () => console.log('wrote', name));
}

const T = {
  grassA: hex('#4a7c40'), grassB: hex('#457539'), grassD: hex('#3e6b35'), grassL: hex('#568a49'),
  water: hex('#33628f'), waterL: hex('#41799f'), waterD: hex('#2b5379'),
  sand: hex('#b99e6b'), sandD: hex('#a58a58'), sandL: hex('#c8af7e'),
  road: hex('#7a755f'), roadD: hex('#6a654f'), roadL: hex('#8a8570'),
  asphalt: hex('#3f3f45'), asphaltD: hex('#36363b'), asphaltL: hex('#4a4a52'), paint: hex('#a8a88a'),
  floor: hex('#8a6a48'), floorD: hex('#77593a'),
  wall: hex('#4c3a28'), wallL: hex('#63503a'), wallD: hex('#382a1c'),
  trunk: hex('#5c3f26'), leaf: hex('#2e5e33'), leafL: hex('#3d7542'), leafHL: hex('#4f8a52'),
  rock: hex('#77777d'), rockL: hex('#92929a'), rockD: hex('#5c5c62'),
  bedFrame: hex('#5c3f26'), bedSheet: hex('#7a8a99'), bedPillow: hex('#c8c8bd'),
  mat: hex('#6a5a3a'), matL: hex('#7d6c48'),
};
...
// ── tiles.png ─────────────────────────────────────────────────────────────
{
  ...
  // 0 grass A / 1 grass B
  for (const [col, base, seed] of [[0, T.grassA, 11], [1, T.grassB, 77]]) {
  ...
  // 10-11 rows 0-1: tree 32×32
  {
    ...
    for (let j = 0; j < 22; j++)
      for (let i = 0; i < 26; i++) {
        const dx = i - 13, dy = j - 11;
        if (dx * dx * 0.85 + dy * dy * 1.2 < 120 + r() * 14) px(png, x + 3 + i, y + 1 + j, T.leaf);
      }
    for (let j = 0; j < 12; j++)
      for (let i = 0; i < 14; i++) {
        const dx = i - 7, dy = j - 6;
        if (dx * dx + dy * dy < 34 + r() * 6) px(png, x + 6 + i, y + 3 + j, T.leafL);
      }
...
const SHIRTS = ['#8a3a3a', '#3a5a8a', '#3a7a4a', '#8a6a2a', '#6a4a8a', '#2a7a7a', '#8a4a6a', '#5a6a2a'];
{
  const rows = 15;
  ...
  const mul = (c, f) => [Math.round(c[0] * f), Math.round(c[1] * f), Math.round(c[2] * f), 255];

  SHIRTS.forEach((s, row) => {
    for (const frame of [0, 1]) drawBody(frame * C, row * C, frame, hex(s), skin, hex('#3a2a1a'), null);
  });
  // row 8: zombie
  for (const frame of [0, 1]) drawBody(frame * C, 8 * C, frame, hex('#3f4a38'), hex('#7fa062'), hex('#2f3a2a'), null);
  // row 9: military
  for (const frame of [0, 1]) drawBody(frame * C, 9 * C, frame, hex('#50603f'), hex('#c9a276'), null, hex('#3a4632'));
  // row 10: trader
  for (const frame of [0, 1]) drawBody(frame * C, 10 * C, frame, hex('#6a5a3a'), hex('#d8a878'), hex('#8a8a8a'), null);
...
];
{
  ...
  const dark = hex('#22201c'), steel = hex('#8a8a92'), steelD = hex('#5c5c64'), woodC = hex('#7a5a34');
  const draw = {
    pistol(x) { rect(png, x + 3, 6, 9, 3, dark); rect(png, x + 4, 9, 3, 4, steelD); rect(png, x + 10, 5, 2, 1, steel); },
    smg(x) { rect(png, x + 2, 6, 11, 3, dark); rect(png, x + 5, 9, 2, 4, steelD); rect(png, x + 9, 9, 2, 3, dark); rect(png, x + 12, 5, 2, 2, steelD); },
    shotgun(x) { rect(png, x + 1, 7, 13, 2, woodC); rect(png, x + 6, 6, 8, 2, dark); rect(png, x + 2, 9, 4, 2, woodC); },
    rifle(x) { rect(png, x + 1, 7, 14, 2, dark); rect(png, x + 2, 9, 3, 3, woodC); rect(png, x + 7, 9, 2, 4, steelD); rect(png, x + 12, 5, 2, 2, dark); },
    spear(x) { for (let i = 0; i < 10; i++) px(png, x + 3 + i, 12 - i, woodC); rect(png, x + 12, 2, 2, 2, steel); px(png, x + 13, 1, steel); },
    axe(x) { for (let i = 0; i < 9; i++) px(png, x + 4 + i, 13 - i, woodC); rect(png, x + 10, 2, 4, 4, steel); rect(png, x + 12, 3, 2, 4, steelD); },
    pickaxe(x) { for (let i = 0; i < 9; i++) px(png, x + 4 + i, 13 - i, woodC); rect(png, x + 9, 2, 6, 2, steel); rect(png, x + 8, 3, 2, 2, steelD); rect(png, x + 13, 4, 2, 2, steelD); },
    ammo_9mm(x) { for (const [bx, by] of [[4, 5], [7, 5], [10, 5]]) { rect(png, x + bx, by, 2, 5, hex('#c8a84a')); px(png, x + bx, by, hex('#8a6a2a')); } },
    ammo_shell(x) { for (const [bx, by] of [[4, 4], [9, 4]]) { rect(png, x + bx, by, 3, 7, hex('#a83a3a')); rect(png, x + bx, by + 5, 3, 2, hex('#c8a84a')); } },
    ammo_556(x) { for (const [bx, by] of [[3, 4], [6, 4], [9, 4], [12, 4]]) { rect(png, x + bx, by, 2, 7, hex('#b09048')); px(png, x + bx, by, hex('#6a5a2a')); } },
    cloth(x) { rect(png, x + 3, 4, 10, 8, hex('#b8b0a0')); rect(png, x + 3, 7, 10, 1, hex('#98907f')); rect(png, x + 3, 10, 10, 1, hex('#98907f')); px(png, x + 12, 4, hex('#98907f')); },
    scrap(x) { rect(png, x + 3, 6, 6, 4, steel); rect(png, x + 8, 4, 5, 3, steelD); rect(png, x + 5, 10, 7, 3, hex('#6a5a4a')); px(png, x + 4, 5, steelD); },
    wood(x) { rect(png, x + 2, 6, 12, 3, woodC); rect(png, x + 2, 10, 12, 3, hex('#6a4c2a')); rect(png, x + 2, 6, 2, 3, hex('#9a7a4c')); rect(png, x + 2, 10, 2, 3, hex('#9a7a4c')); },
    stone(x) { rect(png, x + 4, 6, 8, 6, T.rock); rect(png, x + 5, 5, 5, 3, T.rockL); rect(png, x + 5, 11, 6, 1, T.rockD); },
    bandage(x) { rect(png, x + 3, 5, 10, 6, hex('#e0ded2')); rect(png, x + 3, 7, 10, 2, hex('#c0beb2')); rect(png, x + 6, 5, 4, 6, hex('#d0685a')); },
    medkit(x) { rect(png, x + 3, 4, 10, 9, hex('#b84a42')); rect(png, x + 7, 5, 2, 7, hex('#e8e8e0')); rect(png, x + 5, 8, 6, 2, hex('#e8e8e0')); },
    backpack_mk2(x) { rect(png, x + 4, 4, 8, 9, hex('#7a6a3a')); rect(png, x + 5, 6, 6, 4, hex('#8a7a48')); rect(png, x + 6, 2, 4, 2, hex('#5a4c2a')); },
    backpack_mk3(x) { rect(png, x + 3, 3, 10, 11, hex('#5a5442')); rect(png, x + 4, 5, 8, 5, hex('#6c6650')); rect(png, x + 6, 1, 4, 2, hex('#443f30')); rect(png, x + 4, 11, 3, 2, hex('#443f30')); rect(png, x + 9, 11, 3, 2, hex('#443f30')); },
    helmet_scrap(x) { rect(png, x + 4, 5, 8, 5, steel); rect(png, x + 3, 8, 10, 2, steelD); rect(png, x + 5, 3, 6, 2, steel); px(png, x + 6, 6, steelD); px(png, x + 10, 7, steelD); },
    helmet_military(x) { rect(png, x + 4, 4, 8, 5, hex('#4a5a3a')); rect(png, x + 3, 7, 10, 2, hex('#3a4a2e')); rect(png, x + 5, 2, 6, 2, hex('#4a5a3a')); rect(png, x + 3, 9, 2, 2, hex('#3a4a2e')); rect(png, x + 11, 9, 2, 2, hex('#3a4a2e')); },
    vest_light(x) { rect(png, x + 4, 3, 8, 10, hex('#8a7a5a')); rect(png, x + 5, 5, 6, 3, hex('#9a8a68')); rect(png, x + 5, 9, 6, 2, hex('#7a6a4c')); },
    vest_military(x) { rect(png, x + 4, 3, 8, 10, hex('#44503a')); rect(png, x + 5, 5, 6, 2, hex('#525f46')); rect(png, x + 5, 8, 2, 3, hex('#38422e')); rect(png, x + 9, 8, 2, 3, hex('#38422e')); },
    attach_reddot(x) { rect(png, x + 4, 8, 8, 3, dark); rect(png, x + 6, 4, 4, 4, dark); rect(png, x + 7, 5, 2, 2, hex('#d84a3a')); },
    attach_suppressor(x) { rect(png, x + 2, 6, 12, 4, dark); rect(png, x + 3, 7, 10, 1, steelD); rect(png, x + 13, 7, 2, 2, steelD); },
    fishing_rod(x) { for (let i = 0; i < 11; i++) px(png, x + 2 + i, 13 - i, woodC); px(png, x + 13, 2, steelD); px(png, x + 13, 4, steelD); px(png, x + 13, 6, steelD); px(png, x + 12, 8, hex('#d84a3a')); },
    raw_fish(x) { rect(png, x + 3, 7, 9, 3, hex('#7a94a8')); px(png, x + 12, 6, hex('#7a94a8')); rect(png, x + 12, 7, 3, 3, hex('#5c7488')); px(png, x + 4, 8, hex('#1c1814')); },
    cooked_fish(x) { rect(png, x + 3, 7, 9, 3, hex('#c08a4a')); rect(png, x + 12, 7, 3, 3, hex('#a06c34')); rect(png, x + 5, 8, 2, 1, hex('#8a5a28')); },
    raw_meat(x) { rect(png, x + 4, 5, 8, 7, hex('#b84a52')); rect(png, x + 5, 6, 3, 3, hex('#d87a80')); rect(png, x + 10, 9, 2, 2, hex('#f0ead8')); },
    cooked_meat(x) { rect(png, x + 4, 5, 8, 7, hex('#8a5228')); rect(png, x + 5, 6, 4, 2, hex('#a86c3a')); rect(png, x + 10, 9, 2, 2, hex('#e8d8b8')); },
    canteen(x) { rect(png, x + 5, 4, 6, 9, hex('#5a6a52')); rect(png, x + 6, 2, 4, 3, hex('#3c4636')); rect(png, x + 7, 1, 2, 2, steelD); rect(png, x + 6, 7, 4, 3, hex('#48543f')); },
    canteen_full(x) { rect(png, x + 5, 4, 6, 9, hex('#5a6a52')); rect(png, x + 6, 2, 4, 3, hex('#3c4636')); rect(png, x + 7, 1, 2, 2, steelD); rect(png, x + 6, 7, 4, 3, hex('#4a90c8')); },
    // placeable kits — a crate marked with a mini-icon of the structure
    kit_firepit(x) { rect(png, x + 2, 5, 12, 9, hex('#7a5a34')); rect(png, x + 2, 5, 12, 2, hex('#8a6a3c')); rect(png, x + 6, 8, 4, 4, hex('#d8722a')); px(png, x + 7, 7, hex('#f0b83a')); },
    kit_furnace(x) { rect(png, x + 2, 5, 12, 9, hex('#7a5a34')); rect(png, x + 2, 5, 12, 2, hex('#8a6a3c')); rect(png, x + 5, 7, 6, 5, hex('#6a6a72')); rect(png, x + 6, 9, 4, 2, hex('#d8722a')); },
    kit_workbench(x) { rect(png, x + 2, 5, 12, 9, hex('#7a5a34')); rect(png, x + 2, 5, 12, 2, hex('#8a6a3c')); rect(png, x + 5, 8, 6, 3, hex('#8a683c')); rect(png, x + 5, 7, 3, 1, steel); },
    kit_chest(x) { rect(png, x + 2, 5, 12, 9, hex('#7a5a34')); rect(png, x + 2, 5, 12, 2, hex('#8a6a3c')); rect(png, x + 5, 8, 6, 4, hex('#a8712f')); px(png, x + 8, 9, hex('#d8a24a')); },
    // base-building pieces — drawn as the piece itself so the hotbar reads at a glance
    kit_floor_wood(x) { rect(png, x + 2, 4, 12, 9, hex('#9a7a4c')); rect(png, x + 2, 7, 12, 1, hex('#84673d')); rect(png, x + 2, 10, 12, 1, hex('#84673d')); rect(png, x + 7, 4, 1, 3, hex('#84673d')); rect(png, x + 5, 8, 1, 2, hex('#84673d')); },
    kit_floor_stone(x) { rect(png, x + 2, 4, 12, 9, hex('#8a8a90')); rect(png, x + 2, 8, 12, 1, hex('#6e6e74')); rect(png, x + 8, 4, 1, 4, hex('#6e6e74')); rect(png, x + 5, 9, 1, 4, hex('#6e6e74')); },
    kit_wall(x) { for (const lx of [3, 6, 9, 12]) { rect(png, x + lx, 3, 2, 11, hex('#7a5a34')); px(png, x + lx, 3, hex('#8a683c')); } rect(png, x + 3, 13, 11, 1, hex('#4a331e')); },
    kit_door(x) { rect(png, x + 4, 2, 8, 12, hex('#7a5a34')); rect(png, x + 4, 2, 8, 1, hex('#8a683c')); rect(png, x + 7, 2, 1, 12, hex('#5c4426')); rect(png, x + 4, 7, 8, 1, hex('#5c4426')); rect(png, x + 10, 8, 1, 2, hex('#c8a84a')); },
    kit_fence(x) { rect(png, x + 3, 5, 2, 8, hex('#7a5a34')); rect(png, x + 11, 5, 2, 8, hex('#7a5a34')); rect(png, x + 1, 7, 14, 2, hex('#8a683c')); rect(png, x + 1, 10, 14, 1, hex('#6a4c2a')); },
    kit_torch(x) { rect(png, x + 7, 6, 2, 8, hex('#5c3f26')); rect(png, x + 6, 3, 4, 3, hex('#d8722a')); rect(png, x + 7, 1, 2, 3, hex('#f0b83a')); px(png, x + 7, 0, hex('#f8dc72')); },
    bow(x) {
      // curved limb + string + nocked arrow
      for (let i = 0; i < 12; i++) { const b = Math.round(Math.sin((i / 11) * Math.PI) * 3); px(png, x + 4 + b, 2 + i, woodC); px(png, x + 5 + b, 2 + i, hex('#8a683c')); }
      for (let i = 0; i < 12; i++) px(png, x + 4, 2 + i, hex('#d8d2b8')); // string
      rect(png, x + 5, 7, 8, 1, hex('#9a7a4c')); // arrow shaft
      px(png, x + 13, 7, steel); px(png, x + 12, 6, steel); px(png, x + 12, 8, steel); // head
      px(png, x + 5, 6, hex('#c25047')); px(png, x + 5, 8, hex('#c25047')); // fletching
    },
    arrow(x) {
      for (const [ax, ay] of [[3, 4], [8, 6]]) {
        rect(png, x + ax, ay, 9, 1, hex('#9a7a4c'));
        px(png, x + ax + 9, ay, steel); px(png, x + ax + 8, ay - 1, steel); px(png, x + ax + 8, ay + 1, steel);
