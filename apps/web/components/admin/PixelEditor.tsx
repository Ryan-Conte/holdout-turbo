...
function normalizedAsset(asset: PixelAsset): PixelAsset {
  const width = Math.max(
    1,
    Math.min(128, Math.floor(Number(asset.width) || 16)),
  );
  const height = Math.max(
    1,
    Math.min(128, Math.floor(Number(asset.height) || 16)),
  );
  const size = width * height;
  const legacyPixels = Array.isArray(asset.pixels) ? asset.pixels : [];
  const sourceFrames =
    Array.isArray(asset.frames) && asset.frames.length
      ? asset.frames
      : legacyPixels.length === size
        ? [legacyPixels]
        : [];
  const frames = sourceFrames
    .filter(
      (frame): frame is string[] =>
        Array.isArray(frame) && frame.length === size,
    )
  ...
  return {
    ...
    source: asset.source
      ? {
          ...asset.source,
          frames:
            asset.source.frames ?? (asset.source.sheet === "chars" ? 2 : 1),
        }
      : undefined,
  };
}
...
export function PixelEditor() {
  ...
  const frames = !asset
    ...
    : asset.frames?.length
      ...
      : Array.isArray(asset.pixels) &&
          asset.pixels.length === asset.width * asset.height
        ? [asset.pixels]
        : [new Array<string>(asset.width * asset.height).fill(EMPTY)];
  const frame =
    frames[Math.min(frameIndex, Math.max(0, frames.length - 1))] ?? [];
  const pickerRgb = /^#[0-9a-f]{8}$/i.test(color)
    ? color.slice(0, 7)
    : "#000000";
  const pickerAlpha = /^#[0-9a-f]{8}$/i.test(color)
  ...
  const visibleAssets = document.assets
    ...
    .filter(
      ({ entry }) =>
        !search ||
        `${entry.name} ${entry.id} ${entry.source?.sheet ?? ""}`
          .toLowerCase()
          .includes(search),
    );
  ...
  useEffect(() => {
    ...
    if (onionSkin && frameIndex > 0)
      drawPixels(ctx, frames[frameIndex - 1], asset.width, 0.24);
  ...
  useEffect(() => {
    ...
    const render = (time: number) => {
      const index = playing
        ? Math.floor(time / previewMs) % frames.length
        : frameIndex;
      ctx.clearRect(0, 0, asset.width, asset.height);
  ...
  const updateAsset = (nextAsset: PixelAsset) =>
    setDocument((current) => ({
      ...current,
      assets: current.assets.map((entry, index) =>
        index === selected ? normalizedAsset(nextAsset) : entry,
      ),
    }));

  const updateFrame = (pixels: string[]) => {
    if (!asset) return;
    const nextFrames = frames.map((entry, index) =>
      index === frameIndex ? pixels : entry,
    );
  ...
  const pixelAt = (event: React.PointerEvent<HTMLCanvasElement>) => {
    ...
    const x = Math.floor(
      ((event.clientX - rect.left) * asset.width) / rect.width,
    );
    const y = Math.floor(
      ((event.clientY - rect.top) * asset.height) / rect.height,
    );
  ...
  const applyTool = (
    ...
    const erase =
      event.button === 2 || (event.buttons & 2) !== 0 || tool === "eraser";
    ...
    if (tool === "fill" && !erase)
      updateFrame(floodFill(frame, index, validRgba(color)));
    else {
  ...
  const addAsset = () => {
    ...
    const width = Math.max(
      8,
      Math.min(64, Number(window.prompt("Width in pixels (8-64)", "24")) || 24),
    );
    const height = Math.max(
      8,
      Math.min(
        64,
        Number(window.prompt("Height in pixels (8-64)", "24")) || 24,
      ),
    );
    const pixels = new Array<string>(width * height).fill(EMPTY);
    const next: PixelAsset = {
      id,
      name: id,
      width,
      height,
  ...
  const addFrames = (blank: boolean) => {
    ...
    const count = Math.min(
      Math.max(1, newFrameCount | 0),
      16,
      64 - frames.length,
    );
    if (count <= 0) {
      setStatus("This asset already has the maximum of 64 frames");
      return;
    }
    ...
    historyRef.current = [];
    setStatus(
      `Added ${count} ${blank ? "blank" : "duplicated"} frame${count === 1 ? "" : "s"} to ${asset.name}`,
    );
  };
  ...
  const transformFrame = (mode: "flip-x" | "flip-y" | "clear") => {
    ...
    checkpoint();
    if (mode === "clear") {
      updateFrame(new Array<string>(asset.width * asset.height).fill(EMPTY));
      return;
    }
  ...
  const updatePickerColor = (rgb: string) =>
    setColor(`${rgb}${pickerAlpha.toString(16).padStart(2, "0")}`);
  const updatePickerAlpha = (alpha: number) =>
    setColor(
      `${pickerRgb}${Math.max(0, Math.min(255, alpha)).toString(16).padStart(2, "0")}`,
    );
  const addPaletteColor = () => {
    const next = validRgba(color);
    setColor(next);
    setDocument((current) =>
      current.palette.includes(next)
        ? current
        : { ...current, palette: [...current.palette, next].slice(-64) },
    );
  };
    ...
    <section className="engine-editor pixel-studio">
      <header className="engine-editor-head">
        <div>
          ...
          <p>
            Draw RGBA art, build animation frames, preview movement, then
            publish reusable assets to the engine.
          </p>
        </div>
        <div className="engine-revisions">
          <span>
            GRID {asset?.width ?? 0}x{asset?.height ?? 0} / {frames.length}{" "}
            FRAMES
          </span>
          <small>draft r{revision}</small>
        </div>
      </header>
      <div className="pixel-layout">
        <aside className="engine-records pixel-assets">
          ...
          <div className="pixel-asset-search">
            <input
              ...
              placeholder="Search name, ID or sheet..."
              aria-label="Search sprite assets"
            />
            {assetSearch && (
              <button onClick={() => setAssetSearch("")}>CLEAR</button>
            )}
            <small>
              {visibleAssets.length} MATCH
              {visibleAssets.length === 1 ? "" : "ES"}
            </small>
          </div>
          {visibleAssets.map(({ entry, index }) => (
            ...
            >
              ...
              <small>
                {entry.id} / {Math.max(1, entry.frames?.length ?? 1)}f
              </small>
            </button>
          ))}
          {visibleAssets.length === 0 && (
            <div className="pixel-no-assets">
              NO ASSETS MATCH
              <br />
              <b>{assetSearch}</b>
            </div>
            ...
            <>
              <div className="pixel-title">
                <input
                  value={asset.name}
                  onChange={(event) =>
                    updateAsset({ ...asset, name: event.target.value })
                  }
                />
                <span>{asset.id}</span>
              ...
              <div className="pixel-tool-row">
                {(["pencil", "eraser", "fill", "dropper"] as PixelTool[]).map(
                  (entry) => (
                    <button
                      key={entry}
                    ...
                    >
                      {entry === "dropper"
                        ? "EYEDROPPER (I)"
                        : `${entry.toUpperCase()} (${entry[0].toUpperCase()})`}
                    </button>
                  ),
                )}
                <button onClick={undo}>UNDO</button>
                <button onClick={() => transformFrame("flip-x")}>FLIP X</button>
                <button onClick={() => transformFrame("flip-y")}>FLIP Y</button>
                <button onClick={() => transformFrame("clear")}>CLEAR</button>
              </div>
              <canvas
                ref={canvasRef}
                className="pixel-canvas"
                onPointerDown={(event) => {
                  lastPixelRef.current = -1;
                  applyTool(event, true);
                  event.currentTarget.setPointerCapture(event.pointerId);
                }}
                onPointerMove={(event) => applyTool(event, false)}
                onPointerUp={(event) => {
                  lastPixelRef.current = -1;
              ...
              <div className="pixel-timeline">
                ...
                <button disabled={frames.length <= 1} onClick={deleteFrame}>
                  DELETE CURRENT
                </button>
              </div>
              <div className="pixel-frame-builder">
                <div>
                  <span>ADD EXTRA FRAMES</span>
                  <small>{frames.length}/64 USED</small>
                </div>
                <label>
                  COUNT
                  <input
                    type="number"
                    min={1}
                    max={Math.min(16, Math.max(1, 64 - frames.length))}
                    value={newFrameCount}
                    onChange={(event) =>
                      setNewFrameCount(
                        Math.max(
                ...
                <button
                  disabled={frames.length >= 64}
                  onClick={() => addFrames(false)}
                >
                  + DUPLICATE CURRENT
                </button>
                <button
                  disabled={frames.length >= 64}
                  onClick={() => addFrames(true)}
                >
                  + BLANK FRAMES
                </button>
              </div>
              <div className="pixel-hint">
                Pencil / erase / flood fill / eyedropper. Right-drag erases.
                Ctrl+Z restores the current frame.
              </div>
            </>
          ) : (
        ...
        <aside className="pixel-tools">
          ...
          <div className="pixel-preview-options">
          ...
          <div className="pixel-palette">
            {document.palette.map((entry) => (
              <button
                key={entry}
                className={color === entry ? "active" : ""}
          ...
          <div className="pixel-color-picker">
            ...
            <div
              className="pixel-color-preview"
              style={{ background: validRgba(color) }}
              title={validRgba(color)}
            />
          </div>
          <label>
            ALPHA <b>{Math.round((pickerAlpha / 255) * 100)}%</b>
            <input
              type="range"
              min={0}
              max={255}
              value={pickerAlpha}
              onChange={(event) =>
                updatePickerAlpha(Number(event.target.value))
              }
            />
          </label>
          <label>
            ...
            <input
              ...
              onBlur={() => setColor(validRgba(color))}
              spellCheck={false}
            />
          </label>
